<form class="md:w-1/2" action="" wire:submit.prevent='editarVacante'>
    {{-- TITULO DE VACANTE --}}
    <div>
    ...
    <div class="mt-4">
        ...
        <x-text-input id="imagen" class="block mt-1 w-full p-3" type="file" wire:model="imagen_nueva"
            accept="image/*" />
        <div class="md:grid md:grid-flow-col-dense gap-3 flex justify-center">
            @if ($imagen_nueva)
                <div>
                    <p>Imagen Nueva</p>
                    <img src="{{ $imagen_nueva->temporaryUrl() }}" alt="preview_image">
                </div>
            @endif
            <div>
                <p>Foto actual</p>
                <img src="{{ asset('storage/vacantes/' . $imagen) }}" alt="{{ 'Imagen_vacante' . $imagen }}">
            </div>
        </div>
